...
QWidget *TimeDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QObject::connect(dataTimeEdit,SIGNAL(timeChanged(QTime)),this,SLOT(setData(QTime)));
    return dataTimeEdit;
}
...
void TimeDelegate::setData(QTime val)
{
    emit commitData(dataTimeEdit);
}
QString TimeDelegate::displayText ( const QVariant & value, const QLocale & locale ) const {
        return QStyledItemDelegate::displayText(value.toTime().toString("hh:mm"), locale);
}